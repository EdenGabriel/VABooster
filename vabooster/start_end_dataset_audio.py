...
from vabooster.span_utils import span_xx_to_cxw
...
class StartEndDataset_audio(Dataset):
    ...
    def _get_audio_feat_by_vid(self, vid):
        ...
        # shift_steps=0: 'MR-full-mAP',48.12 'HL-min-VeryGood-mAP', 41.07 'HL-min-VeryGood-Hit1', 67.81
        # shift_steps=1: 'MR-full-mAP',47.96 'HL-min-VeryGood-mAP', 41.06 'HL-min-VeryGood-Hit1', 67.81
        # shift_steps=5: 'MR-full-mAP',47.99 'HL-min-VeryGood-mAP', 41.07 'HL-min-VeryGood-Hit1', 67.68
        # shift_steps=15: 'MR-full-mAP',47.98 'HL-min-VeryGood-mAP', 41.02 'HL-min-VeryGood-Hit1', 67.81
        # shift_steps=30: 'MR-full-mAP',48.07 'HL-min-VeryGood-mAP', 41.07 'HL-min-VeryGood-Hit1', 68.19
        
        # shift_steps=-1: 'MR-full-mAP',48.14 'HL-min-VeryGood-mAP', 41.10 'HL-min-VeryGood-Hit1', 68.0
        # shift_steps=-5: 'MR-full-mAP',48.23 'HL-min-VeryGood-mAP', 41.11 'HL-min-VeryGood-Hit1', 68.06
        # shift_steps=-15: 'MR-full-mAP',48.09 'HL-min-VeryGood-mAP', 40.99 'HL-min-VeryGood-Hit1', 67.61
        # shift_steps=-30: 'MR-full-mAP',48.02 'HL-min-VeryGood-mAP', 41.05 'HL-min-VeryGood-Hit1', 67.68
        # a_feat = shift_1d_feature(a_feat,shift_steps=-30,pad_mode='edge') 
        return torch.from_numpy(a_feat)  # (D, ) or (Lq, D)


def shift_1d_feature(feat: np.ndarray, shift_steps: int, pad_mode='edge'):
    """
    对 2D 特征序列 [T, D] 在时间维度做全局偏移
    shift_steps: 正数=向右移(滞后)，负数=向左移(超前)
    pad_mode: 'zero' 或 'edge'
    """
    T, D = feat.shape
    if shift_steps == 0:
        return feat.copy()
    ...
    if pad_mode == 'zero':
        ...
        out = np.tile(feat[0:1], (T, 1)) if shift_steps < 0 else np.tile(feat[-1:], (T, 1))
    else:
        raise ValueError("pad_mode must be 'zero' or 'edge'")

    if shift_steps > 0:
        # 前 shift_steps 帧填充，后 T-shift_steps 帧拷贝
        out[shift_steps:] = feat[:T-shift_steps]
    else:
        k = -shift_steps
        out[:T-k] = feat[k:]
    return out
