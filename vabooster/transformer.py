...
def calculate_l1_norm(f):
    ...
    f = f / (f_norm + 1e-9)
    return f
def my_random_walk(x, y, w,lambda_z=5.0):
    ...
    # print(eye_x.shape,affinity_mat.shape,mat_inv_x.shape,y2x_sum_x.shape)
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        if self.args.cross_cal_type == "cross_attention":
            t2v_encoder_layer = T2V_TransformerEncoderLayer(d_model, nhead, dim_feedforward,
                                                    dropout, activation, normalize_before, 0)
    ...
    def forward(self, src, mask, query_embed, pos_embed, video_length=None, moment_idx=None, msrc=None, mpos=None, mmask=None,
        ...
        refpoint_embed = query_embed.unsqueeze(1).repeat(1, bs, 1)  # (#queries, batch_size, d)
         ...
         # Saliency Token
        if self.args.cross_cal_type == 'random_walk':
            # t2t_src = my_random_walk(src[video_length:].permute(1, 0, 2), src[video_length:].permute(1, 0, 2), 0.5,lambda_z=5.0) # self.w
            # v2v_src = my_random_walk(src[:video_length].permute(1, 0, 2), src[:video_length].permute(1, 0, 2), 0.5,lambda_z=5.0) # self.w
            # t2v_src = my_random_walk(v2v_src, t2t_src, 0.5,lambda_z=5.0) # self.w
            
            t2v_src = my_random_walk(src[:video_length].permute(1, 0, 2), src[video_length:].permute(1, 0, 2), 0.5,lambda_z=5.0) # self.w
            t2v_src = t2v_src.permute(1, 0, 2)
            attn_weights = torch.matmul(src[:video_length].permute(1,0,2),src[video_length:].permute(1,2,0))
            attn_weights = torch.nn.functional.softmax(attn_weights, dim=-1)
        elif self.args.cross_cal_type == 'cross_attention':
            t2v_src, attn_weights = self.t2v_encoder(src, src_key_padding_mask=mask, pos=pos_embed, video_length=video_length, dummy=False)  # (L, batch_size, d)

        # Saliency Token
        if self.args.global_cal_type == 'random_walk':
            gtoken_refine = my_random_walk(gtoken.unsqueeze(0).repeat(src.shape[1],1,1), ctxtoken, 0.5,lambda_z=5.0) # self.w
            src_ = gtoken_refine.sum(1).unsqueeze(1).permute(1, 0, 2)
            ...
            frame_importance = attn_weights[:, :, :].sum(2).clone().detach()  # b 75
            ### Masking empty clips
...
class T2V_TransformerEncoderLayer(nn.Module):
    ...
    def forward_pre(self, src,
                    src_mask: Optional[Tensor] = None,
                    src_key_padding_mask: Optional[Tensor] = None,
                    pos: Optional[Tensor] = None, dummy=False):
        pass
