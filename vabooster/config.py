...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--model_ema", action="store_false", help="default: set to use")
        parser.add_argument("--ema_decay", type=float, default=0.8, help="EMA decay")
        parser.add_argument("--global_cal_type", type=str, default='random_walk', choices=["cross_attention", 'random_walk'], help="global calculation method")
        parser.add_argument("--cross_cal_type", type=str, default='random_walk', choices=["cross_attention", 'random_walk'], help="cross method")
        parser.add_argument("--audio_cat", action="store_true", help="default: no cat")
        ...
        parser.add_argument('--num_prompts', default=1, type=int,
                            help="Number of selected prompt tokens")
        ...
        parser.add_argument("--lw_distill", type=float, default=1.,
                            help="weight for saliency loss, set to 0 will ignore")
        parser.add_argument('--span_loss_coef', default=1, type=float) # 10
        parser.add_argument('--giou_loss_coef', default=1, type=float) # 1
        parser.add_argument('--label_loss_coef', default=1, type=float) # 4
        ...
        if isinstance(self, TestOptions):
            ...
            opt.results_dir = os.path.join(opt.results_root,
                                           "-".join([opt.dset_name, ctx_str, opt.exp_id,
                                                     str(opt.enc_layers) + str(opt.dec_layers) + str(opt.moment_layers) + str(opt.sent_layers),
                                                     'nprom_' + str(opt.num_prompts) + '_' + str(opt.total_prompts)]))
            mkdirp(opt.results_dir)
            save_fns = ['vabooster/model.py', 'vabooster/transformer.py']
